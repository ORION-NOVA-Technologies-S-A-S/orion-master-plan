# Indicadores Clave de Desempeño (KPI) de ORION NOVA Technologies S.A.S.

| Documento | Información |
|-----------|-------------|
| Código | ONT-STR-006 |
| Nombre | Indicadores Clave de Desempeño (KPI) |
| Versión | 1.0.0 |
| Estado | En desarrollo |
| Fecha | Agosto de 2026 |
| Elaborado por | Carlos Culma |
| Revisión | Pendiente |
| Aprobación | Pendiente |

---

# Introducción

Los Indicadores Clave de Desempeño (Key Performance Indicators - KPI) constituyen el conjunto de métricas utilizadas por ORION NOVA Technologies S.A.S. para medir el avance, desempeño y cumplimiento de los objetivos establecidos en el Plan Estratégico 2026–2035.

Los KPI permiten transformar la estrategia corporativa en resultados medibles, facilitando la toma de decisiones basada en datos y promoviendo una cultura organizacional orientada a la mejora continua.

---

# Objetivo

Definir un conjunto de indicadores cuantitativos y cualitativos que permitan evaluar el desempeño estratégico, financiero, comercial, tecnológico y operativo de ORION NOVA Technologies S.A.S., asegurando el cumplimiento de su visión empresarial.

---

# Alcance

Los indicadores definidos en este documento serán aplicables a todas las áreas estratégicas de la organización, incluyendo:

- Dirección General.
- Investigación e Innovación.
- Desarrollo de Software.
- Inteligencia Artificial.
- Infraestructura Tecnológica.
- Ciberseguridad.
- Comercial y Marketing.
- Finanzas.
- Recursos Humanos.

---

# Principios de los KPI

Los indicadores definidos por ORION deberán cumplir las siguientes características:

- Específicos.
- Medibles.
- Alcanzables.
- Relevantes.
- Temporales.
- Objetivos.
- Verificables.
- Comparables en el tiempo.

---

# Clasificación de los KPI

Los indicadores se agrupan en las siguientes categorías:

- Estratégicos
- Financieros
- Clientes
- Desarrollo Tecnológico
- Investigación e Innovación
- Ciberseguridad
- Recursos Humanos
- Calidad
- Operaciones

---

# KPI Estratégicos

Estos indicadores permiten medir el cumplimiento de la visión corporativa y del Plan Estratégico.

| Indicador | Descripción | Meta |
|------------|-------------|------|
| Cumplimiento del Plan Estratégico | Porcentaje de iniciativas estratégicas ejecutadas |  90% |
| Productos del Ecosistema ORION desarrollados | Productos oficialmente lanzados |  8 % |
| Nuevos mercados alcanzados | Países donde opera la empresa |  5% |
| Alianzas estratégicas | Convenios activos con organizaciones |  15% |
| Proyectos de investigación ejecutados | Proyectos I+D+i finalizados |  5 % por año |

---

# KPI Financieros

Estos indicadores evalúan el crecimiento económico y la sostenibilidad de la organización.

| Indicador | Descripción | Meta |
|------------|-------------|------|
| Crecimiento anual de ingresos | Incremento porcentual anual |  20% |
| Margen de utilidad operacional | Rentabilidad operacional |  25% |
| Flujo de caja | Balance entre ingresos y egresos | Positivo |
| ROI de proyectos | Retorno de inversión |  30% |
| Diversificación de ingresos | Número de líneas de negocio activas |  5 % |

---

# KPI Comerciales

Estos indicadores permiten evaluar el desempeño comercial.

| Indicador | Descripción | Meta |
|------------|-------------|------|
| Clientes nuevos | Nuevos clientes por año |  20 % |
| Clientes activos | Clientes con contratos vigentes |  100% |
| Tasa de retención | Permanencia de clientes |  90% |
| Nivel de satisfacción | Encuestas de satisfacción |  95% |
| Tiempo promedio de respuesta comercial | Atención de solicitudes |  24 horas |

---

# KPI de Desarrollo Tecnológico

Estos indicadores miden la calidad del desarrollo de software.

| Indicador | Descripción | Meta |
|------------|-------------|------|
| Disponibilidad de plataformas | Uptime anual |  99.9% |
| Cobertura de pruebas | Código cubierto por pruebas |  85% |
| Bugs críticos por versión | Incidentes críticos liberados |  3% |
| Tiempo promedio de despliegue | Duración del proceso de despliegue |  30 minutos |
| Tiempo de respuesta de APIs | Promedio de respuesta |  250 ms |

---

# KPI de Inteligencia Artificial e Innovación

Estos indicadores permiten evaluar la capacidad innovadora de ORION.

| Indicador | Descripción | Meta |
|------------|-------------|------|
| Modelos de IA desarrollados | Modelos implementados |  10 por año |
| Nuevas funcionalidades inteligentes | Funciones basadas en IA |  5 por año |
| Precisión promedio de modelos | Exactitud de los modelos |  92% |
| Publicaciones científicas | Artículos publicados |  2 por año |
| Patentes o registros tecnológicos | Registros obtenidos |  1 cada 3 años |

---

# KPI de Ciberseguridad

Estos indicadores permiten controlar la seguridad de la infraestructura tecnológica.

| Indicador | Descripción | Meta |
|------------|-------------|------|
| Incidentes críticos de seguridad | Ataques exitosos | 0 |
| Equipos actualizados | Infraestructura con parches al día | 100% |
| Copias de seguridad exitosas | Backups realizados correctamente | 100% |
| Tiempo de recuperación | Recuperación ante incidentes |  4 horas |
| Vulnerabilidades críticas abiertas | Hallazgos pendientes | 0 |

---

# KPI de Recursos Humanos

Estos indicadores permiten evaluar el crecimiento del talento humano.

| Indicador | Descripción | Meta |
|------------|-------------|------|
| Rotación del personal | Porcentaje anual |  10% |
| Horas de capacitación | Formación por colaborador |  40 horas/año |
| Certificaciones obtenidas | Certificaciones profesionales |  5 por año |
| Clima organizacional | Encuestas internas |  90% |
| Participación en innovación | Colaboradores vinculados a proyectos I+D+i |  70% |

---

# KPI de Calidad

Estos indicadores permiten asegurar la mejora continua.

| Indicador | Descripción | Meta |
|------------|-------------|------|
| Auditorías satisfactorias | Auditorías aprobadas | 100% |
| No conformidades críticas | Hallazgos críticos | 0 |
| Cumplimiento documental | Documentación actualizada | 100% |
| Procesos estandarizados | Procesos documentados | 100% |
| Acciones de mejora implementadas | Acciones ejecutadas |  90% |

---

# Herramientas de Medición

Para el seguimiento de los indicadores se utilizarán herramientas tecnológicas que permitan recopilar, visualizar y analizar la información.

Entre ellas:

- GitHub.
- GitHub Projects.
- GitHub Actions.
- PostgreSQL.
- Docker.
- Grafana.
- Prometheus.
- Power BI.
- Google Analytics.
- Jira o Trello.

---

# Frecuencia de Seguimiento

| Categoría | Frecuencia |
|------------|------------|
| Estratégicos | Trimestral |
| Financieros | Mensual |
| Comerciales | Mensual |
| Desarrollo Tecnológico | Mensual |
| Inteligencia Artificial | Trimestral |
| Ciberseguridad | Mensual |
| Recursos Humanos | Trimestral |
| Calidad | Trimestral |

---

# Responsables

| Área | Responsable |
|------|-------------|
| Dirección Estratégica | CEO |
| Tecnología | CTO |
| Desarrollo de Software | Director de Desarrollo |
| Investigación | Director de I+D+i |
| Comercial | Director Comercial |
| Finanzas | Director Financiero |
| Calidad | Responsable de Calidad |
| Seguridad | CISO |

---

# Seguimiento y Evaluación

Los KPI serán revisados periódicamente por la Dirección General y los responsables de cada área con el fin de:

- Evaluar el cumplimiento del Plan Estratégico.
- Identificar oportunidades de mejora.
- Detectar desviaciones respecto a las metas.
- Definir acciones correctivas y preventivas.
- Actualizar los indicadores conforme al crecimiento de la empresa.

Los resultados serán consolidados en un informe ejecutivo para la alta dirección.

---

# Conclusión

Los Indicadores Clave de Desempeño constituyen una herramienta fundamental para garantizar que ORION NOVA Technologies S.A.S. avance de manera sostenible hacia el cumplimiento de su visión empresarial.

El seguimiento continuo de estos indicadores permitirá fortalecer la innovación, optimizar los procesos, mejorar la calidad de los productos y asegurar la competitividad de la organización en el mercado tecnológico.

---

## Trazabilidad

- ONT-STR-000 - Resumen Ejecutivo
- ONT-STR-001 - Plan Estratégico 2026–2035
- ONT-STR-002 - Análisis FODA
- ONT-STR-005 - Roadmap Estratégico
- ONT-STR-007 - Objetivos y Resultados Clave (OKR)
- ONT-STR-008 - Gestión de Riesgos

---

## Control de Cambios

| Versión | Fecha | Descripción | Autor |
|----------|--------|-------------|--------|
| 1.0.0 | Agosto 2026 | Creación inicial del documento de Indicadores Clave de Desempeño (KPI). | Carlos Culma |

---

**Documento oficial de ORION NOVA Technologies S.A.S.**